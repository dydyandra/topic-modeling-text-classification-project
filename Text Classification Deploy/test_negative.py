...
def negative():
    if request.method == "POST":
        first_name = request.form.get("text_predict")
        sequence = tokenizer.texts_to_sequences([first_name])
        padded = pad_sequences(sequence, MAX_LEN_NEG)
        pred = model_neg.predict(padded)
        prediction = neg_labels[np.argmax(pred)]
        return render_template("negative.html", pred=" Topic of the above text is {}".format(prediction))
